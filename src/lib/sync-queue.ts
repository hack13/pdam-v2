...
export async function getSyncBoss() {
  if (!bossPromise) {
    ...
    bossPromise = boss.start().then(async () => {
      await boss.createQueue(SYNC_QUEUE, { notify: true, retryLimit: 5, retryDelay: 30, retryBackoff: true, retryDelayMax: 43_200, heartbeatSeconds: 60, expireInSeconds: 48 * 3600, deleteAfterSeconds: 30 * 86400 });
...
export async function enqueueConnectionSync(connectionId: string, userId: string) {
  ...
  const boss = await getSyncBoss();
  const jobId = await boss.send(SYNC_QUEUE, { runId: run.id, connectionId, userId } satisfies SyncJobData, { retryLimit: 5, retryDelay: 30, retryBackoff: true, retryDelayMax: 43_200, heartbeatSeconds: 60, expireInSeconds: 48 * 3600, group: { id: userId } });
