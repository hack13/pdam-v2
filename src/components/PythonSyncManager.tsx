...
export function PythonSyncManager() {
  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl border border-white/10 bg-white/[0.03] p-5"><h2 className="font-medium text-white">How to use the Python client</h2><ol className="mt-3 list-decimal space-y-2 pl-5 text-sm text-zinc-400"><li>Download <code>tailcache-sync.py</code> below and save it somewhere easy to find.</li><li>Open Terminal, PowerShell, or Command Prompt in that folder.</li><li>Run the script with your TailCache URL, sync token, and optional local folder:</li></ol><code className="mt-3 block overflow-x-auto rounded-lg bg-black/30 px-3 py-2 text-sm text-indigo-200">python3 tailcache-sync.py https://tailcache.org pdam_sync_… ./tailcache-backup</code><p className="mt-3 text-xs text-zinc-500">On Windows, use <code>python</code> instead of <code>python3</code> if needed. The script creates the folder, verifies file hashes, skips unchanged files, and never deletes local files.</p></section>
    <section className="rounded-xl border border-white/10 bg-white/5 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-medium text-white">Python sync script</h2><p className="mt-1 text-sm text-zinc-400">Save this script as <code>tailcache-sync.py</code> and run it with your TailCache URL and sync token.</p></div><a href="/pdam-sync.py" download="tailcache-sync.py" className="rounded-lg border border-white/10 px-3 py-2 text-sm text-indigo-300 hover:bg-white/5">Download Python script</a></div><pre className="mt-4 max-h-[36rem] overflow-auto rounded-lg border border-white/10 bg-black/30 p-4 text-xs leading-5 text-zinc-300"><code>{script}</code></pre></section>
  </div>;
}
