...
export function BackupSyncManager() {
  ...
  const [form, setForm] = useState<Record<string, string>>({ name: '', endpoint: '', bucket: '', region: 'auto', accessKeyId: '', secretAccessKey: '', username: '', password: '', rootPath: '' });
  ...
  async function addConnection(event: React.FormEvent) {
    ...
    const response = await fetch('/api/sync/connections', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ providerType, ...form }) });
    const data = await response.json(); setMessage(response.ok ? 'Destination connected.' : data.error ?? 'Could not connect destination.');
    if (response.ok) { setForm({ name: '', endpoint: '', bucket: '', region: 'auto', accessKeyId: '', secretAccessKey: '', username: '', password: '', rootPath: '' }); await load(); }
    setBusy(false);
  }
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      <form onSubmit={addConnection} className="space-y-3 rounded-xl border border-white/10 bg-white/5 p-4">
        ...
        {providerType === 's3' ? <><input required value={form.bucket} onChange={(event) => update('bucket', event.target.value)} placeholder="Bucket" className="w-full rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm text-white placeholder-zinc-500" /><div className="grid grid-cols-2 gap-3"><input required value={form.accessKeyId} onChange={(event) => update('accessKeyId', event.target.value)} placeholder="Access key" className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm text-white placeholder-zinc-500" /><input required type="password" value={form.secretAccessKey} onChange={(event) => update('secretAccessKey', event.target.value)} placeholder="Secret key" className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm text-white placeholder-zinc-500" /></div></> : <div className="grid grid-cols-2 gap-3"><input required value={form.username} onChange={(event) => update('username', event.target.value)} placeholder="Username" className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm text-white placeholder-zinc-500" /><input required type="password" value={form.password} onChange={(event) => update('password', event.target.value)} placeholder="Password" className="rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm text-white placeholder-zinc-500" /></div>}
